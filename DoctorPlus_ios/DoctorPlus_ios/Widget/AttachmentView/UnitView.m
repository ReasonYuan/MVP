...
#define defaultWidth  50      // 每一个unitCell的默认宽度
#define defaultPace   8       // unitCell之间的间距
#define duration      0.5     // 动画执行时间
#define defaultVisibleCount 9 //默认显示的unitCell的个数
...
- (void) setProperty
{
    ...
    _scrollView.frame = CGRectMake(0, (self.bounds.size.height - 62)/2.0, self.bounds.size.width - defaultPace, 70);
...
{
      _scrollView.contentSize = [self contentSizeForUIScrollView:(_unitList.count + 3)];
      [_scrollView scrollRectToVisible:CGRectMake(_scrollView.contentSize.width, 0, defaultWidth, self.frame.size.height) animated:YES];
}
...
- (void) addNewUnit:(NSString *)icon withName:(NSString *)name
{
    ...
    newUnitCell = [[UnitCell alloc] initWithFrame:CGRectMake(x, 0, defaultWidth, 62) andIcon:icon andName:name];
...
- (void)unitCellTouched:(UnitCell *)unitCell
{
    ...
    if (_unitList.count  > defaultVisibleCount
            && (_unitList.count - index ) <= defaultVisibleCount) {
        _frontMove = YES;
    }
    if (index == _unitList.count  && !_frontMove)
        _defaultUnit.alpha = 0.5;
    ...
    [_delegate itemDelete:unitCell];
}

- (void) unitCellClicked:(UnitCell *)unitCell{
    [_delegate itemClicked:unitCell];
}
