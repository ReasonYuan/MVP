...
+(CATransform3D)carousel:(iCarousel *)carousel itemTransformForOffset:(CGFloat)offset baseTransform:(CATransform3D)transform itmeView:(UIView*)view
{
    //高度比例（289） 30 : 35 : 40 : 44 : 51 : 44 : 40 : 35 : 30
    CGFloat realeOffet = fabs(offset);
    CGFloat stage = carousel.frame.size.height*51/289.0;
    CGFloat nextStage = carousel.frame.size.height*44/289.0;
    CGFloat height = stage;
    CGFloat centerY  = 0;
    CGFloat startCenterY = 0;
    CGFloat alpha = 1.f;
    do{
        if(realeOffet == 0 ) {
            break;
        ...
        if(realeOffet <= 1.0){
            CGFloat off = realeOffet;
            height = stage - ( stage - nextStage ) * off;
            centerY = startCenterY + ( stage + nextStage ) / 2.0 * off;
            break;
        }
        startCenterY += ( stage + nextStage ) / 2.0;
        stage = nextStage;
        nextStage = carousel.frame.size.height*40/289.0;
        if(realeOffet <= 2.0){
            CGFloat off = realeOffet - 1.0;
            height = stage - ( stage - nextStage ) * off;
            centerY = startCenterY + ( stage + nextStage ) / 2.0 * off;
            break;
        }
        startCenterY += ( stage + nextStage ) / 2.0;
        stage = nextStage;
        nextStage = carousel.frame.size.height*35/289.0;
        if(realeOffet <= 3.0){
            CGFloat off = realeOffet - 2.0;
            height = stage - ( stage - nextStage ) * off;
            centerY = startCenterY + ( stage + nextStage ) / 2.0 * off;
            break;
        }
        startCenterY += ( stage + nextStage ) / 2.0;
        stage = nextStage;
        nextStage = carousel.frame.size.height*30/289.0;
        if(realeOffet <= 4.0){
            CGFloat off = realeOffet - 3.0;
            alpha = 1 - off;
            height = stage - ( stage - nextStage ) * off;
            centerY = startCenterY + ( stage + nextStage ) / 2.0 * off;
            break;
        }
        height = carousel.frame.size.height*28/289.0;
        centerY = carousel.frame.size.height*2;
    }while(false);
    if(offset < 0){
        centerY = -centerY;
    }
    CGFloat scale = height / (carousel.frame.size.height*51/289.0);
    CGFloat width = carousel.frame.size.width * scale;
    transform = CATransform3DTranslate(transform, (carousel.frame.size.width-width) / 2.0, centerY, 0);
...
- (CATransform3D)transformForItemViewWithOffset:(CGFloat)offset withView:(UIView*)view
{
    ...
    switch (_type)
    {
        case iCarouselTypeCustom:
        {
            return [_delegate carousel:self itemTransformForOffset:offset baseTransform:transform itmeView:view];
        }
        case iCarouselTypeLinear:
        {
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //calculate transform
    CATransform3D transform = [self transformForItemViewWithOffset:offset withView:view];
