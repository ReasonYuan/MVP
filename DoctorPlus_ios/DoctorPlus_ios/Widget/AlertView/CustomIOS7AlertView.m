...
- (void)show
{
    ...
    // Attached to the top most window (make sure we are using the right orientation):
        ...
        do{
            if(window){
                UINavigationController* na = nil;
                if([window.rootViewController isKindOfClass:[UINavigationController class]]){
                    na = (UINavigationController*)window.rootViewController;
                }else{
                    na = window.rootViewController.navigationController;
                }
                if(na){
                    UIViewController* currentController = [na.viewControllers lastObject];
...
- (UIView *)createContainerView
{
    ...
    CGSize dialogSize = containerView.frame.size;//[self countDialogSize];
    ...
    dialogContainer.layer.cornerRadius = cornerRadius;red:
    dialogContainer.layer.borderColor = [UIColor grayColor].CGColor;
...
- (void)keyboardWillShow: (NSNotification *)notification
{
    ...
    [UIView animateWithDuration:0.2f delay:0.0 options:UIViewAnimationOptionTransitionNone
					 animations:^{
                         dialogView.frame = CGRectMake((screenSize.width - dialogSize.width) / 2, (screenSize.height - keyboardSize.height - dialogSize.height) / 2, dialogSize.width, containerView.frame.size.height);//dialogSize.height);
					 }
					 completion:nil
	 ];
}
...
- (void)keyboardWillHide: (NSNotification *)notification
{
    ...
    [UIView animateWithDuration:0.2f delay:0.0 options:UIViewAnimationOptionTransitionNone
					 animations:^{
                         dialogView.frame = CGRectMake((screenSize.width - dialogSize.width) / 2, (screenSize.height - dialogSize.height) / 2, dialogSize.width, containerView.frame.size.height);//dialogSize.height);
					 }
					 completion:nil
	 ];
}
...
- (void)setCanCelOnTouchOutSide:(BOOL)isCancel
{
    _isCancelOnTouchOutSide = isCancel;
}

- (void)setCancelonTouch:(BOOL)isCancel
{
    _isCancelonTouchSide = isCancel;
}

-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (_isCancelonTouchSide) {
        [self close];
        return;
    }
    
    if (!_isCancelOnTouchOutSide)return;
    
    UITouch* touch = [touches anyObject];
    CGPoint point = [touch locationInView:containerView];
    
    float x = containerView.frame.origin.x;
    float y = containerView.frame.origin.y;
    float w = containerView.frame.size.width;
    float h = containerView.frame.size.height;
