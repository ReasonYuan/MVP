...
public final class CameraManager {
	...
	private static final int MIN_FRAME_WIDTH = 160;//240,640
	private static final int MIN_FRAME_HEIGHT = 160;
	private static final int MAX_FRAME_WIDTH = 600;// 480;
...
//		int minLength = Math.min(screenResolution.x, screenResolution.y);
//		if (framingRect == null) {
//			if (camera == null) {
//				return null;
//			}
//			int width = minLength * 3 / 4;// screenResolution.x
...
//			int topOffset = screenResolution.y/4;//(screenResolution.y - height) / 3;
...
//		return framingRect;
	...
	boolean mIsSetPrevieRect = false;
	...
	public Rect getFramingRectInPreview() {
		if(!mIsSetPrevieRect){
			...
			Point screenResolution = configManager.getScreenResolution();
			int mPreviewWidth = screenResolution.x;
			int mPreviewHeigth = screenResolution.y;
			if (mPreview != null && mPreview.getWidth() != 0) {
				mPreviewWidth = mPreview.getWidth();
				mPreviewHeigth = mPreview.getHeight();
				mPreview.getLocationOnScreen(mLocation);
				mIsSetPrevieRect = true;
			}
			float scaleX = cameraResolution.y / (float) screenResolution.x;
			float scaleY = (cameraResolution.x / (float) screenResolution.y);
			int left = (int) (rect.left * scaleX);
			int top = (int) ((rect.top - mLocation[1]) * scaleY * (screenResolution.y / (float) mPreviewHeigth));
			int with = (int) (rect.width() * scaleX);
			int height = (int) (rect.height() * scaleY * (screenResolution.y / (float) mPreviewHeigth));
			rect.set(left, top, left + with, top + height);
