...
public class AsyncHttpClient {
    ...
    private static SchemeRegistry getDefaultSchemeRegistry(boolean fixNoHttpResponseException, int httpPort, int httpsPort) {
        if (fixNoHttpResponseException) {
            FQLog.d(LOG_TAG, "Beware! Using the fix is insecure, as it doesn't verify SSL certificates.");
        }

        if (httpPort < 1) {
            httpPort = 80;
            FQLog.d(LOG_TAG, "Invalid HTTP port number specified, defaulting to 80");
        }

        if (httpsPort < 1) {
            httpsPort = 443;
            FQLog.d(LOG_TAG, "Invalid HTTPS port number specified, defaulting to 443");
        }
        ...
        if (fixNoHttpResponseException){
            ...
            sslSocketFactory.setHostnameVerifier(new X509HostnameVerifier() {
				...
				public void verify(String hostname, String[] arg1, String[] arg2) throws SSLException {
					throw new RuntimeException("Unhandled ssl verify 1!");
				}
				
				@Override
				public void verify(String hostname, X509Certificate arg1) throws SSLException {
					throw new RuntimeException("Unhandled ssl verify 2!");
				}
				...
				public void verify(String hostname, SSLSocket sslSocket) throws IOException {
					sslSocket.addHandshakeCompletedListener(new HandshakeCompletedListener() {
						...
						public void handshakeCompleted(HandshakeCompletedEvent event) {
							try {
								Certificate[] cers = event.getPeerCertificates();
								YiyiLiveServerVerify.verifyCertificate(cers);
							} catch (Exception e) {
								throw new RuntimeException(new SSLException(e));
							}
						}
					});
				}
				...
				public boolean verify(String hostname, SSLSession sslSession) {
					throw new RuntimeException("Unhandled ssl verify 4!");
				}
			});
        }
    ...
    protected RequestHandle sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, ResponseHandlerInterface responseHandler, Context context) {
        ...
        if(responseHandler.getRequestHeaders() != null){
        	for (int i = 0; i < responseHandler.getRequestHeaders().length; i++) {
        		uriRequest.addHeader(responseHandler.getRequestHeaders()[i]);
			}
        }
//        uriRequest.addHeader(HEADER_CONTENT_TYPE, RequestParams.APPLICATION_JSON);
